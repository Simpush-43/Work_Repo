...
export const CreateCoupen = async (req,res)=>{
  try {
    ...
    res.status(201).json({ message: "Coupen created successfully", Coupen: coupen });
  } catch (err) {
    res.status(400);
...
export const GetCoupen = async (req,res)=>{
  try {
    ...
    res.status(201).json({ message: "Coupen got successfully", GetCoupen: coupens });
  } catch (err) {
    res.status(400);
...
export const DeleteCoupen = async (req,res)=>{
  try {
    ...
    res.status(400);
    throw new Error(err.response?.data?.message || "Failed to delete the coupens");
  }
}
...
export const ToggleCoupen = async (req,res)=>{
  try {
    ...
    await coupen.save()
    res.status(201).json({ message: "Coupen got toggled successfully",toggledcoupen:coupen});
  } catch (err) {
